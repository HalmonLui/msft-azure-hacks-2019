...
def max_sharpe_ratio(mean_returns, cov_matrix, risk_free_rate):
    ...
    result = sp.optimize.minimize(neg_sharpe_ratio, num_assets*[1./num_assets,], args=args, method='SLSQP', bounds=bounds, constraints=constraints)
...
def min_variance(mean_returns, cov_matrix):
    ...
    bound = (0.0, 1.0)
    bounds = tuple(bound for asset in range(num_assets))
    result = sp.optimize.minimize(portfolio_volatility, num_assets*[1./num_assets,], args=args, method='SLSQP', bounds=bounds, constraints=constraints)
    return result
...
def efficent_frontier(mean_returns, cov_matrix, returns_range):
    efficients = []
